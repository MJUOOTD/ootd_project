...
- 상단 박스 영역
  - 로그인 전: "로그인하세요" 버튼 박스 노출
  - 로그인 후: 
    - 왼쪽: 동그란 프로필 사진 placeholder
    - 오른쪽: 사용자 이름 텍스트 

- 하단 메뉴 영역
  - 로그아웃
  - 설정 (SettingsScreen 이동)
  - [추가 가능] 개인정보 처리방침, 고객센터 등

- UX 고려
  - 로그인 여부에 따라 UI 상태 분기
  - 로그아웃 시 → "로그인하세요" 박스로 복귀
...
### 3.7 알림 화면 (NotificationScreen)

- 진입 경로:
  - 홈 화면 우측 상단 알림 아이콘 클릭
...
| 구분 | 항목 | 세부 내용 | 상태 |
|------|------|-----------|------|
| 온보딩 | 앱 소개 슬라이드 | 3장 구성, 드래그 넘김 | ☐ 미구현 |
| 온보딩 | 회원가입/로그인 | 이메일 + 소셜 로그인 | ☐ 미구현 |
| 온보딩 | 권한 요청 | 알림, 위치 권한 요청 | ☐ 미구현 |
| 온보딩 | 권한 거부 처리 | “위치 권한 켜주세요” 안내 화면 | ☐ 미구현 |
| 온보딩 | 초기 프로필 설정 | 이름, 이메일, 성별, 체온 민감도 | ☐ 미구현 |
| 홈 화면 | 시간대 인사말 | 아침/오후/저녁 구분 출력 | ☐ 미구현 |
| 홈 화면 | 날씨 위젯 | 기상청 API 연동, 30분 캐시 | ☐ 미구현 |
| 홈 화면 | 코디 추천 캐러셀 | 날씨+체온 민감도 기반, 3개 노출 | ☐ 미구현 |
| 홈 화면 | 스크롤/캐러셀 위치 복원 | 상세→홈 복귀 시 위치 복원 | ☐ 미구현 |
| 홈 화면 | 스켈레톤 UI | 로딩 시 표시 | ☐ 미구현 |
| 피드백 | 피드백 시트 진입 조건 | 상세 화면 체류 후 노출 | ☐ 미구현 |
| 피드백 | 착용 선택 | 추천 3개 중 1개 선택 | ☐ 미구현 |
| 피드백 | 체감 온도 선택 | 추움/적당/더움 | ☐ 미구현 |
| 피드백 | 하트 저장 | 옷장(Saved)에 저장 | ☐ 미구현 |
| 피드백 | 이따가 할래요 | defer 처리 | ☐ 미구현 |
| 피드백 | 추천받은 옷 안 입음 | not_worn 처리 | ☐ 미구현 |
| 검색 화면 | 검색창 | 텍스트 검색 | ☐ 미구현 |
| 검색 화면 | 필터 옵션 | 스타일/가격대/브랜드 | ☐ 미구현 |
| 검색 화면 | 검색 결과 뷰 | 그리드 레이아웃 | ☐ 미구현 |
| 저장함 | 저장 목록 표시 | 그리드 레이아웃 | ☐ 미구현 |
| 저장함 | 하트 토글 | 저장/해제 | ☐ 미구현 |
| 알림 | 알림 목록 | 리스트 뷰 | ☐ 미구현 |
| 알림 | 읽음 처리 | 개별/전체 읽음 | ☐ 미구현 |
| 알림 | 카운트 표시 | 아이콘 뱃지 | ☐ 미구현 |
| 설정 | 프로필 수정 | 이름/성별/체온 민감도 | ☐ 미구현 |
| 설정 | 개인화 설정 변경 | 온보딩 데이터 수정 | ☐ 미구현 |
| 홈 화면  | 알림 아이콘   | 우측 상단 배치, 알림 화면 이동 | ☐ 미구현 |
| MY 화면 | 로그인 전 박스 | "로그인하세요" 표시        | ☐ 미구현 |
| MY 화면 | 로그인 후 박스 | 프로필 사진 + 이름 표시     | ☐ 미구현 |
| MY 화면 | 로그아웃     | 세션 종료              | ☐ 미구현 |
| MY 화면 | 설정       | 설정 화면 이동           | ☐ 미구현 |
| 설정 | 로그아웃 | 세션 종료 | ☐ 미구현 |
| UI/UX | 다크모드 | 지원 여부 | ☐ 미구현 |
| UI/UX | 반응형 디자인 | 다양한 화면 대응 | ☐ 미구현 |
